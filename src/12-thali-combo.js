...
export function createThaliDescription(thali) {
  if (typeof thali !== "object" || thali === null || typeof thali.name !== "string" || !Array.isArray(thali.items) ||
...
export function getThaliStats(thalis) {
  ...
  const prices = thalis.map(t => t?.price ?? 0);
...
export function generateThaliReceipt(customerName, thalis) {
  ...
  const total = thalis.reduce((sum, t) => sum + (typeof t.price === "number" ? t.price : 0), 0);
